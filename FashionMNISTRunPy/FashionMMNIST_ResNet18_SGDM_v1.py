...
def maincode(args):
    ...
    filename='FashionMNIST_'+'A_'+'ResNet18'+'_penalty_'+str(l2)+'_e_'+str(num_epochs)+         '_N_'+str(n_nodes)+'_SGDM'+'_lr_'+str(learning_rate)+'_beta_'+str(beta)
    if method=='none':
        filename=filename
    ...
    # Define the model
    model = models.resnet18(pretrained=False, num_classes=num_output)
    ...
    isExist = os.path.exists('./FashionMNIST_ResNet18_CSV/')
    if not isExist:
   # Create a new directory because it does not exist
       os.makedirs('./FashionMNIST_ResNet18_CSV/')
    results_df.to_csv('./FashionMNIST_ResNet18_CSV/'+filename+'.csv', index=True)
    # Close Tensorboard
    writer.flush()
    writer.close()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train a neural network on FashionMNIST ResNet18')
